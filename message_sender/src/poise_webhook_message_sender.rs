...
use crate::traits::{TimesMessageSender, TimesMessage};
...
impl TimesMessageSender for PoiseWebhookMessageSender {
    ...
    async fn send_all(&self, message: TimesMessage, times: Vec<repository::UtTime>) -> Result<(), Self::Error> {
        // Webhookを送るだけなら，トークンとやらはなしでもいいらしい
        let http = Http::new("");

        for time in times.into_iter() {
            let webhook = Webhook::from_url(&http, &time.webhook_url).await?;
            let builder = ExecuteWebhook::new().content(message.content.clone())
